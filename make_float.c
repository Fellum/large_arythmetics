...
unsigned	*fill_float_f(unsigned long mask, int exp, unsigned long mant)
{
	unsigned *cur;
	unsigned *res;
	...
	if (exp < 0)
	{
		...
		mask = (unsigned)exp > F_MANT - 1 ? 0 : mask >> (unsigned)exp;
	while (mask)
	{
		mul_dig_longnum(cur, 5, cur);
		mul_dig_longnum(res, 10, res);
...
unsigned	*fill_integer_f(unsigned long mask, int exp, unsigned long mant)
{
	unsigned	*res;
	unsigned	*pow2;
	int			cur;
	...
	if (exp >= 0)
	{
		mask = mask >> (exp > F_MANT ? F_MANT - 1 : (exp - 1));
		cur = exp - F_MANT;
		while (cur-- > 0)
			mul_dig_longnum(pow2, 2, pow2);
		while (mask <= F_MASK && exp > 0)
		{
			if (mant & mask)
...
char		*make_float(float val, int precision)
{
	...
	res = NULL;
	tmp.f = val;
	integer_part = fill_integer_f(F_MASK, tmp.parts.exp - 127, tmp.parts
			.mant);
	float_part = fill_float_f(F_MASK, tmp.parts.exp - 127, tmp.parts.mant);
	apply_precision(integer_part, float_part, precision);
	append_string(&res, int_part_to_str(integer_part, tmp.parts.sign));
